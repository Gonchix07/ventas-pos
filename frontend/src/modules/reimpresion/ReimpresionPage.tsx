...
import { reimpresion, type ComprobanteReimpresion } from "../../shared/api/reimpresion";
...
export function ReimpresionPage() {
  ...
  const buscar = async () => {
    ...
    try {
      setResultados(await reimpresion.buscar(idSucursal, texto.trim(), desde || undefined, hasta || undefined));
    } catch (e) {
  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-main">
        ...
        <p className="muted">
          Buscá la factura o nota de crédito por número, cliente o CUIT. Se imprime en pantalla
          (papel común) — si el comprobante original salió por controlador fiscal, esto NO reimprime
          el rollo fiscal original.
        </p>
        ...
        {resultados && (
          <table className="grid">
            <thead>
              <tr><th>Comprobante</th><th>Tipo</th><th>Fecha</th><th>Cliente</th><th>Total</th><th>Estado</th><th /></tr>
            </thead>
            <tbody>
              {resultados.map((c) => (
