...
public record ErpCodBarraRow(
    long IdPresentacionErp, string Codigo, string TipoCodigo, DateTime FechaModificacion);
...
/// en cualquiera de las tres cuenta como "el artículo cambió".</summary>
public interface IErpArticuloReader
{
    Task<IReadOnlyList<ErpArticuloRow>> GetArticulosModificadosAsync(DateTime watermark, int loteSize, CancellationToken ct);
    Task<IReadOnlyList<ErpPresentacionRow>> GetPresentacionesModificadasAsync(DateTime watermark, int loteSize, CancellationToken ct);
    Task<IReadOnlyList<ErpCodBarraRow>> GetCodBarrasModificadosAsync(DateTime watermark, int loteSize, CancellationToken ct);
}
...
{
    Task<IReadOnlyList<ErpClienteRow>> GetClientesModificadosAsync(DateTime watermark, int loteSize, CancellationToken ct);
}
